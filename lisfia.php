...
window.open("lisfiapdf.php?fechadescuento="+fechadescuento,"parte1","top=0,left=395,status=no,toolbar=no,scrollbar=yes,location=no,type=fullWindow,fullscreen");	// los primeros 500 socios	width=385,height=180,
/*
...
if (!$accion) {
	echo "<div id='div1'>";
	echo "<form action='lisfia.php?accion=Listado' name='form1' method='post'>";
	echo '<fieldset><legend>Información Para Listado de Fiadores</legend>';
	echo 'Reporte de Fiadores al: ';
...
//	$fechadescuento=convertir_fecha($_POST['fechadelpago']);		// revisar que no hayan nominas con esa fecha
	echo "<div id='div1'>";
	echo "<form action='lisfia.php?accion=Listo' name='form1' method='post'>"; //  onsubmit='return realizo_abono(form1)'>";
	$fechadescuento=$_POST['fechadelpago'];
	echo '<fieldset><legend>Recopilando información Para Listado de Fiadores al '.$fechadescuento.'</legend>';
