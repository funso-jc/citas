...
if ((! $_POST['procesar']) and (! $_FILES['archivo']['name'])) {
	echo '<fieldset><legend>Informacion para procesar archivo para Farmacia</legend>';
	echo '<form action="farmacia.php" method="post" name="form1" enctype="multipart/form-data">';
	echo 'Fecha de Descuento Prestamo farmacia ';
?>
...
//	echo '<input type="checkbox" name="nominasemanal" value = "1" checked/> Nomina Semanal<br />';
	echo 'Indique el archivo (CSV) que contiene la informacion de Farmacia<input name="archivo" type="file" value="Examinar"><br>';
...
if(@$_FILES['archivo']['name']!=='') // {
	...
	$ruta_total = $_SERVER['DOCUMENT_ROOT']."/cajalocal/farmacia/".$_FILES['archivo']['name'];
//	echo $ruta_total;
...
*/
			...
			$sql_310="INSERT INTO sgcaf310 (codsoc_sdp, cedsoc_sdp, nropre_sdp, codpre_sdp, f_soli_sdp, f_1cuo_sdp, ultcan_sdp, monpre_sdp, stapre_sdp, cuota, nrocuotas, cuota_ucla, ip) VALUES ('".$r200['cod_prof']."', '".$micedula."', '$elnumero', '004', '$b', '$b', 0, $montolpj, 'A', $cuota, '$nrocuota', $cuota, '$ip')";
//			echo $sql_310.'<br>';
			$saldo=$montolpj;
		}
		else 
