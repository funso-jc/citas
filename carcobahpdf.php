...
// print_r(getdate()).'<br>';
$sql2="CREATE TEMPORARY TABLE moroso SELECT cod_prof, cpc_prof, ape_prof, nombr_prof, ultap_prof, DATEDIFF('".$fechasiguiente."',ultap_prof) AS diaspasados, 1 as solvente FROM ".$_SESSION['bdd']."_sgcaf200 WHERE  UPPER(statu_prof)='ACTIVO' ORDER BY cod_prof ";
$res2=mysql_query($sql2) or die($sql2);
// echo $sql2;
$sql2="DELETE FROM moroso where diaspasados < 0";
$res2=mysql_query($sql2) or die($sql2);
...
//print_r(getdate()).'<br>';
// echo '<br>';
	$solvencia=condicion($r2['cod_prof'],$fechasiguiente, false,$diatraso);
//	echo $diatraso;
	if ($solvencia == false)
	{
		$sqlsol="UPDATE moroso set solvente = 0 where cod_prof = '".$r2['cod_prof']."'";
...
*/
set_time_limit(30);	
// elimino los solventes
$sql2="delete from moroso where solvente = 1";
$res2=mysql_query($sql2) or die($sql2);
...
$header[3]='Nombre(s)';
$header[4]='Ult.Pago';
$header[5]='Tiempo (dias)';
/*
$header[8]='NC/CC';
*/
$alto=3;
$salto=$alto;
$w=array(15,15,50,50,15,20); // ,25,15,15,15); // ,15,20,15,10); // ,25,25,25,25,25,25);
$p[0]=20;
...
$linea=encabeza_l_prestamos($header,$w,$p,$pdf,$salto,$alto,$fechasiguiente);
/*
$pdf->SetY($linea);
$pdf->SetX($p[0]);
...
$primeravez=true;
$gt=0;
$sql_amor="SELECT * from moroso group by cod_prof";
// echo $sql_amor;
$a_amor=mysql_query($sql_amor) or die($sql_amor);
while ($r_amor = mysql_fetch_assoc($a_amor)){
	$linea+=$salto;
	$pdf->SetY($linea);
	$pdf->SetX($p[0]);		$pdf->Cell($w[0],$alto,$r_amor['cod_prof'],0,0,'LRTB',0);
	$pdf->SetX($p[1]);		$pdf->Cell($w[1],$alto,$r_amor["cpc_prof"],0,0,'LRTB',0); 
	$pdf->SetX($p[2]);		$pdf->Cell($w[2],$alto,$r_amor["ape_prof"],0,0,'LRTB',0);
	$pdf->SetX($p[3]);		$pdf->Cell($w[2],$alto,$r_amor["nombr_prof"],0,0,'LRTB',0);
  	$pdf->SetX($p[4]);		$pdf->Cell($w[3],$alto,convertir_fechadmy($r_amor["ultap_prof"]),0,0,'LRTB');
	$pdf->SetX($p[5]);		$pdf->Cell($w[4],$alto,$r_amor["diaspasados"],0,0,'R');
/*
...
$pdf->MultiCell(0,0,"Cartas de Cobranza Ahorros al ".convertir_fechadmy($fechadescuento),0,C,0);
$pdf->SetY($linea);
$pdf->SetFont('Arial','',7);
$linea+=5;
$pdf->SetX(220);
