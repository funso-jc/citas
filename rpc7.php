	...
	if(!$db) {
		...
		if(isset($_POST['queryString'])) {
			...
			if(strlen($queryString) >0) {
				...
				$filtro="SELECT cod_prof, ced_prof, ape_prof,nombr_prof FROM ".$_SESSION['bdd']."_sgcaf200 WHERE ((cod_prof LIKE '$queryString%') or (ape_prof LIKE '$queryString%') or (nombr_prof LIKE '$queryString%')) order by cod_prof  LIMIT 10";
