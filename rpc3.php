	...
	if(!$db) {
		...
		if(isset($_POST['queryString'])) {
			...
			if(strlen($queryString) >0) {
				...
				$filtro="SELECT substr(cue_codigo,18,4) as cue_codigo,cue_nombre FROM ".$_SESSION['bdd']."_sgcaf810 WHERE ((substr(cue_codigo,1,17)='".$_SESSION['nro']."') and ((substr(cue_codigo,18,21) LIKE '$queryString%') or (cue_nombre LIKE '$queryString%'))) and (cue_nivel = '".$niveles ."') order by cue_codigo LIMIT 10";
			    //echo $filtro; 
				...
				if($query) {
					...
					while ($result = $query ->fetch_object()) {
						...
						// YOU MUST CHANGE: $result->value to $result->your_colum
	         			echo '<li onClick="fill3(\''.$result->cue_codigo.'\');">'.$result->cue_codigo.' - '.$result->cue_nombre.'</li>';
	         		}
				} else {
