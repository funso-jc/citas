...
*/
	...
	$debe=$_POST['mdeducciones2'];
	if ($debe > 0)
	{
		agregar_f820($elasiento, $b, '-', $_POST['cdeducciones2'], $_POST['condeducciones2'].$afiliado, $debe, $haber, 0,$ip,0,$referencia,'','S',0);
		$restando+=abs($debe);
	}
...
*/
	...
	echo '</strong> </td>';
	// determinar fianzas
	$sqlfianza="SELECT sum(monto_fia - monlib_fia) AS fianzas FROM sgcaf320 WHERE codfia_fia = '$codigo' AND tipmov_fia = 'F'";
	$resfia=mysql_query($sqlfianza);
	$rfia=mysql_fetch_assoc($resfia);
	$fianzasdebe=$rfia['fianzas'];
	// fin determinar fianzas
	echo '</tr>';
		if ($fila2['porcentaje']!=100){
		    ...
		    echo '<tr><td width="127" scope="col">Max.Socio </td>';
			echo '<td width="127" align="right" scope="col">';
			echo "<input name='ohab_prof' type='text' id='ohab_prof' size='12' maxlength='12' value ='".($fila['hab_f_prof']-$fianzasdebe)."' ".$lectura." />";
		    echo '</td><td width="127" scope="col">Max.UCLA </td>';
			echo '<td width="127" align="right" scope="col">';
	...
	Motivo: <input name="motivo" type="text" id="motivo" size="65" maxlength="65" value="" />
        ...
        <?php
			...
			else echo number_format(($totalahorros-$fianzasdebe),2,'.',','); 
			?> 
    </td>
	</tr>
	<tr><td colspan="8">
...
<?php
	...
	echo '<td><input name="creintegro" type="text" id="creintegro" size="21" maxlength="21" value="" /></td>';
	echo '<td>Concepto</td>';
	...
	echo '<tr><td>Cuenta Contable</td>';
	echo '<td><input name="cdeducciones" type="text" id="cdeducciones" size="21" maxlength="21" value="4-02-99-01-99-01-0001" /></td>';
	echo '<td>Concepto</td>';
...
//	echo '<tr><td>el monto '.$t2.'</td></tr>';
	echo '<tr><td>Cuenta Contable</td>';
	echo '<td><input name="cdeducciones2" type="text" id="cdeducciones2" size="21" maxlength="21" value="4-02-99-01-99-02-0001" /></td>';
	echo '<td>Concepto</td>';
	echo '<td><input name="condeducciones2" type="text" id="condeducciones2" size="40" maxlength="40" value="Gastos Administrativos " /></td>';
	echo '<td>Monto</td>';
	echo '<td><input name="mdeducciones2" type="text" id="mdeducciones2" size="15" maxlength="15" value="'.($t2*0.01).'" /></td>';
	echo '</tr>';
