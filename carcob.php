...
window.open("carcobpdf.php?fechadescuento="+fechadescuento+"&minimo="+minimo+"&maximo="+maximo+"&inicio="+inicio+"&fin="+fin,"parte1","top=0,left=395,status=no,toolbar=no,scrollbar=yes,location=no,type=fullWindow,fullscreen")
//
...
if (!$accion) {
	echo "<div id='div1'>";
	echo "<form action='carcob.php?accion=Listado' name='form1' method='post'>";
...
//	$fechadescuento=convertir_fecha($_POST['fechadelpago']);		// revisar que no hayan nominas con esa fecha
	echo "<div id='div1'>";
	echo "<form action='carcob.php?accion=Cartas' name='form1' method='post'>"; //  onsubmit='return realizo_abono(form1)'>";
...
//	phpinfo();
	...
	echo "<form action='carcob.php?accion=Listo' name='form1' method='post' onsubmit='return realiza_emails_cobranza(form1)'>"; 
	echo "<div id='div1'>";
	echo '<input type="hidden" name="fechadescuento" id="fechadescuento" value="'.$fechadescuento.'">';
	echo '<input type="hidden" name="elminimo" id="elminimo" value="'.$elminimo.'">';
	echo '<input type="hidden" name="elmaximo" id="elmaximo" value="'.$elmaximo.'">';
	echo '<input type="hidden" name="elinicio" id="elinicio" value="'.$elinicio.'">';
	echo '<input type="hidden" name="elfin" id="elfin" value="'.$elfin.'">';
	echo '<input type="submit" name="Submit" value="Enviar emails de Notificacion de Cobranza" />';
/*
