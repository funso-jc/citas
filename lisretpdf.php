...
$sql="select date_format(f_ret_capu,'%Y') AS anoretiro, cod_prof, ced_prof, concat(trim(ape_prof),' ',nombr_prof) as nombre, f_ing_capu, aport_prof, sueld_prof from ".$_SESSION['bdd']."_sgcaf200 where (f_ret_capu >= '$desde' and f_ret_capu  <= '$hasta') order by anoretiro, f_ret_capu, nombre";//   limit 30"; //  limit 20";
// echo $sql;
...
$pdf->MultiCell(0,0,"Retiro de Socios del ".convertir_fechadmy($desde). ' al '.convertir_fechadmy($hasta),0,C,0);
$pdf->SetY($linea);
$pdf->SetFont('Arial','',7);
$linea+=5;
$pdf->SetY($linea);
