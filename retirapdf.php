...
$cuento='En reunión de Junta Directiva según Acta N° ' . $rretiro['nro_acta'] . ' de fecha ' . convertir_fechadmy($rretiro['fecha_acta']).' se acordó la aprobación de un retiro de haberes al socio ';
...
$cuento.='  ****Bs. ('.trim(number_format($rretiro['montoreti'],2,".",",")).')**** El motivo presentado por el socio es: ';
...
$pdf->SetY($linea);
$pdf->SetX(10);
$pdf->Cell(70,6,'Ahorros Capitalizables ',0,0,'L',0);
$pdf->SetX(80);
$pdf->Cell(30,6,number_format(($socio['hab_f_divi']+$rretiro['ret_divi']),2,".",","),0,0,'R',0);
$pdf->SetX(110);
$pdf->Cell(30,6,number_format($rretiro['ret_divi'],2,".",","),0,0,'R',0);
$pdf->SetX(140);
// $pdf->Cell(50,6,number_format(($socio['hab_f_prof']),2,".",","),0,0,'R',0);
$linea+=5;
$pdf->SetY($linea);
...
$pdf->SetX(80);
$montoahorro=$socio['hab_f_prof']+$rretiro['ret_capu']+$socio['hab_f_empr']+$rretiro['ret_ucla']+$socio['hab_f_extr']+$rretiro['ret_volu']+$socio['hab_f_divi']+$rretiro['ret_divi'];
$montoretiro=$rretiro['ret_capu']+$rretiro['ret_ucla']+$rretiro['ret_volu']+$rretiro['ret_divi'];
...
/*
	...
	while ($fila2 = mysql_fetch_assoc($resultado2))
	{
		...
		if ($saldo > 0)
		{
			...
			// recordar cancelar el prestamo y gaurdar la informacion del retiro
			$sql="update sgcaf310 set stapre_sdp='C', renovado=1, renova_por='RETIRO', where cedsoc_sdp='$micedula' and nropre_sdp='".$fila2['nropre_sdp']."'";
//			echo $sql;
//			$salida=mysql_query($sql);
		}
		else 
			$reintegros+=$saldo;
...
$pdf->SetY($linea);
$pdf->SetX(10);
$pdf->Cell(50,6,'Nro Comprobante: '.$elasiento,0,0,'L',0);
$pdf->SetX(60);
$elcheque=$_SESSION['elcheque'];
$pdf->Cell(50,6,'Nro Cheque: '.$elcheque,0,0,'L',0);
$pdf->SetX(110);
$laplaza=$_SESSION['laplaza'];
$pdf->Cell(100,6,'Institución: '.$laplaza,0,0,'L',0);
...
function buscar_saldo_f810_ret($cuenta, $elasiento)
{
	...
	$sql_f820="select com_monto1, com_monto2 from sgcaf820 where ((com_cuenta='$cuenta') and (com_nrocom != '$elasiento')) order by com_fecha";
//	echo $sql_f820;
...
delete from sgcaf700 where registro > 3033;
delete from sgcaf820 where com_nrocom="09060800911";
delete from sgcaf830 where enc_clave="09060800911";
*/
?>
