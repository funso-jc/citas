	...
	if(!$db) {
		...
		if(isset($_POST['queryString'])) {
			...
			if(strlen($queryString) >0) {
				...
				$filtro="SELECT ced_prof, ape_prof, nombr_prof FROM ".$_SESSION['bdd']."_sgcaf200 WHERE ((ced_prof LIKE '$queryString%') or (ape_prof LIKE '$queryString%') or (nombr_prof LIKE '$queryString%')) order by ced_prof LIMIT 10";
			    // echo $filtro; 
...
//				$query = $db->query("SELECT cue_codigo, cue_nombre FROM sgcaf810 WHERE ((cue_codigo LIKE '$queryString%') or (cue_nombre LIKE '$queryString%')) and (cue_nivel = 6) LIMIT 10");
		...
		// and (cue_nivel = ".$_SESSION['maxnivel'].")) 
				if($query) {
					...
					echo '<li onClick="fill5(\''.$result->ced_prof.'\');">'.$result->ced_prof. ' - ' .$result->ape_prof.' - '.$result->nombr_prof.'</li>';
	         		}
				} else {
