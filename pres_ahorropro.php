...
if ($accion == "Renovar") {	// seleccionar el tipo de prestamo nuevo de renovacion
	...
	$temp = "";
	echo "<form enctype='multipart/form-data' action='pres_ahorropro.php?accion=Solicitar' name='form1' id='form1' method='post' onsubmit='return valpre(form1)'";
...
//			die('<h1> Los datos del socio estan desactualizado con '.$losdias.' dias. Actualicelo en la seccion de Asociado/Socios</h1>') ;
		...
		$sql = "SELECT * FROM ".$_SESSION['bdd']."_sgcaf310, ".$_SESSION['bdd']."_sgcaf360 WHERE (cedsoc_sdp = '$estacedula' and stapre_sdp='A' and (! renovado)) and codpre_sdp=cod_pres ORDER BY f_soli_sdp DESC"; // ." LIMIT ".($conta-1).", 10";
		$rs = mysql_query($sql);
...
// 		bucle de listado
		...
		echo "<td class='centro'><a href='pres_ahorropro.php?accion=Ver&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'><img src='imagenes/page_user_dark.gif' width='16' height='16' border='0' title='Consultar' alt='Consultar'/></a></td>";
		echo "<td class='centro'><a href='pres_ahorropro.php?accion=ModificarCuota&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'><img src='imagenes/modiftbl.jpg' width='16' height='16' border='0' title='Modificar Cuota' alt='Modificar Cuota'/></a></td>";
		echo "<td class='centro'>";
		if ($row['renovacion']>1)
			if ($row['ultcan_sdp'] >= $row['renovacion']) {
				echo "<a href='pres_ahorropro.php?accion=Renovar&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'>";
				...
				echo "<a href='pres_ahorropro.php?accion=ReAjustar&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'>";
			...
			//	jc / mendez viejo / mendez nuevo 
			{
				echo "<a href='pres_ahorropro.php?accion=CancelarPrestamo&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'>";
			...
			{
//				echo "<td class='centro'>";
				echo "<a href='pres_ahorropro.php?accion=NetoDepositar&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'>";
...
if (!$accion) {
	echo "<form action='pres_ahorropro.php?accion=Buscar' name='form1' method='post'>";
    echo '  C&eacute;dula ';
...
if (($accion == "Editar") or ($accion=="Renovar")) {	// muestra datos para prestamo
	...
	$temp = "";
	echo "<form enctype='multipart/form-data' action='pres_ahorropro.php?accion=EscogePrestamo' name='form1' method='post' onsubmit='return valsoc(form1)'>";
	$elcodigo="";
	pantalla_prestamo($result,$cedula);
	$result = mysql_query($sql) or die ('Error 200-1 <br>'.$sql.'<br>'.mysql_error());
	$res2=mysql_fetch_assoc($result);
	$elcodigo=$res2['cod_prof'];
	...
	$micedula=substr($cedula,0,4).'.'.substr($cedula,4,3).'.'.substr($cedula,7,4);
	$sqls="select * from ".$_SESSION['bdd']."_suspende where ((cedula = '$micedula') and  (activo = 1) and (now() < suspendido))";
//	echo $sqls;
...
/*
	if ($_SESSION['disponibilidadprestamo'] > 0) {
		if (($elstatus == "ACTIVO") or ($elstatus == "JUBILA")) {
			$sqlprestamos.="select * from ".$_SESSION['bdd']."_sgcaf360 where ";}
		else {
...
//		$sqlprestamos="select * from sgcaf360 where (retab_pres = 0) or (cod_pres='004') or (cod_pres='021') or (cod_pres='011') or (cod_pres='012') or (cod_pres='013') and ";
		$sqlprestamos="select * from ".$_SESSION['bdd']."_sgcaf360 where (retab_pres = 0) or (cod_pres='004') or (cod_pres='005') or (cod_pres='021')  and ";
...
*/
	$sqlprestamos="SELECT * FROM ".$_SESSION['bdd']."_sgcafap1, ".$_SESSION['bdd']."_sgcafapc WHERE (cod_prof = '$elcodigo') and (des_prog = 0) and (substr(cod_prog,1,1)=serie) and (pres_prog = 0) order by cod_prog";
//	echo $sqlprestamos;
	echo '<td>Seleccione Ahorro </td>';
   	echo '<td class="rojo">';
	echo '<select name="elprestamo" size="1">';
//	echo $sqlprestamos;
	$resultado=mysql_query($sqlprestamos);
	while ($fila2 = mysql_fetch_assoc($resultado)) {
		echo '<option value="'.$fila2['cod_prog'].'">'.$fila2['cod_prog'].' - '.$fila2['prestamo'].' - '.$fila2['cuotas'].'</option>'; }
	echo '</select> *'; 
	echo '</td>';
...
/*
	...
	$sqlespecial="select * from ".$_SESSION['bdd']."_sgcaf310, ".$_SESSION['bdd']."_sgcaf360 where (cedsoc_sdp='".$micedula."' and codpre_sdp=cod_pres and stapre_sdp='A' and ! renovado) and (dcto_sem=0) and (f_1cuo_sdp <= '$fechadescuento') order by f_1cuo_sdp";
//	echo $sqlespecial;
	$resultespecial=mysql_query($sqlespecial);
	$fianzase = $afectane = $noafectane = $semanale = $montocuotase = 0;
	while($roww=mysql_fetch_assoc($resultespecial)) {
...
*/
	if (!$_SESSION['numeroarenovar']) echo "<input type = 'submit' value = 'Convertir a Prestamo'></form>\n"; 
...
//	die ('primer dectuentp: '.$_POST['primerdcto']);
	...
	$sql_200="select * from ".$_SESSION['bdd']."_sgcaf200 where ced_prof='$cedula'";
...
//	echo 'aqui viene '.$codigo;
	...
	$sql_acta="select * from ".$_SESSION['bdd']."_sgcafact where acta='$nroacta' and f_dcto='$primerdcto' order by fecha desc limit 1";
//	echo $sql_acta;
...
//**********************************
	$sql="update ".$_SESSION['bdd']."_sgcaf310 set netcheque = $neto_cheque where cedsoc_sdp = '$micedula' and nropre_sdp='$referencia'";
	if ($elprestamo == '055')
		$sql="update ".$_SESSION['bdd']."_sgcaf310 set netcheque = 29999 where cedsoc_sdp = '$micedula' and nropre_sdp='$referencia'";
	if ($elprestamo == '064')
		$sql="update ".$_SESSION['bdd']."_sgcaf310 set netcheque = 19999 where cedsoc_sdp = '$micedula' and nropre_sdp='$referencia'";
	if ($elprestamo == '066')
	{
		$sql="update ".$_SESSION['bdd']."_sgcaf310 set netcheque = 29999 where cedsoc_sdp = '$micedula' and nropre_sdp='$referencia'";
//		echo $sql;
	...
	if ($r_360['albanco']==0)
	{
		$sql="update ".$_SESSION['bdd']."_sgcaf310 set netcheque = 0 where cedsoc_sdp = '$micedula' and nropre_sdp='$referencia'";
		$resultado=mysql_query($sql);
	}
...
if ($accion == "ReAjustar") {	//  para aquellos que solo aumentan el monto y varian la cuota
	$mostrarregresar=1;
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='pres_ahorropro.php?accion=Reajuste' name='form1' id='form1' method='post' onsubmit='return valajuste(form1)'";
	$cedula = $_GET['cedula'];
	...
	$r_200=mysql_fetch_assoc($a_200);
	// busco el prestamo
	$sql_310="select * from ".$_SESSION['bdd']."_sgcaf310, ".$_SESSION['bdd']."_sgcaf360 where (cedsoc_sdp='$micedula') and (stapre_sdp='A' and ! renovado) and (codpre_sdp=cod_pres) and (nropre_sdp = '$elnumero') limit 1";
	//  order by nropre_sdp";
	$a_310=mysql_query($sql_310);
...
if ($accion == "Reajuste") {	//  para aquellos que solo aumentan el monto y varian la cuota
	...
	$r_200=mysql_fetch_assoc($a_200);
	// actualizo 
	$sql_310="update ".$_SESSION['bdd']."_sgcaf310 set monpre_sdp = monpre_sdp + ".$_POST['montoprestamo'].", cuota_ucla = ".$_POST['cuota']." where (nropre_sdp = '$elnumero') and (cedsoc_sdp = '$micedula')";
//	echo $sql_310;
	$a_310=mysql_query($sql_310);
...
if ($accion == "ModificarCuota") {	//  para aquellos que solo aumentan el monto y varian la cuota
	$mostrarregresar=1;
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='pres_ahorropro.php?accion=ModificaCuota' name='form1' id='form1' method='post' onsubmit='return valajuste(form1)'";
	$cedula = $_GET['cedula'];
	$elnumero = $_GET['nropre'];
	$micedula=substr($cedula,0,4).'.'.substr($cedula,4,3).'.'.substr($cedula,7,4);
	$deci=$_SESSION['deci'];
...
if ($accion == "ModificaCuota") {	//  para aquellos que solo aumentan el monto y varian la cuota
	...
	$r_200=mysql_fetch_assoc($a_200);
	// actualizo 
	$sql_310="update ".$_SESSION['bdd']."_sgcaf310 set cuota_ucla = ".$_POST['cuota']." where (nropre_sdp = '$elnumero') and (cedsoc_sdp = '$micedula')";
//	echo $sql_310;
	$a_310=mysql_query($sql_310);
	// busco el prestamo y lo muestro actualizado
	$sql_310="select * from ".$_SESSION['bdd']."_sgcaf310, ".$_SESSION['bdd']."_sgcaf360 where (cedsoc_sdp='$micedula') and (stapre_sdp='A' and ! renovado) and (codpre_sdp=cod_pres) and (nropre_sdp = '$elnumero') limit 1";
...
// phpinfo();
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='pres_ahorropro.php?accion=Concretar' name='form1' id='form1' method='post' onsubmit='return valpreres(form1)'";
...
//	$sql_310="select * from sgcaf310, sgcaf360 where (cedsoc_sdp='$micedula') and (incluir_otros=1) and (codpre_sdp != '$elprestamo') and (stapre_sdp='A' and ! renovado) and (codpre_sdp=cod_pres) order by nropre_sdp";
	$sql_310="select * from ".$_SESSION['bdd']."_sgcaf310, ".$_SESSION['bdd']."_sgcaf360 where (cedsoc_sdp='$micedula') and (incluir_otros=1) and (stapre_sdp='A' and ! renovado) and (codpre_sdp=cod_pres) and (nropre_sdp = '".$_SESSION['numeroarenovar']."') order by nropre_sdp";
	...
	if ($_SESSION['numeroarenovar']) {
		...
		$sql="select cuent_pres,codsoc_sdp,cuent_int from ".$_SESSION['bdd']."_sgcaf310, ".$_SESSION['bdd']."_sgcaf360 where (nropre_sdp='".$_SESSION['numeroarenovar']."') and stapre_sdp='A' and ! renovado and (cedsoc_sdp='$micedula')  and (codpre_sdp=cod_pres)";
//		echo $sql;
//	echo $sql_310;
...
*/
	...
	$sql3="select cue_codigo, substr(cue_codigo,-4) as socio from ".$_SESSION['bdd']."_sgcaf810 where substr(cue_codigo,-4)='$elcodigo' order by cue_codigo";
...
//		if (! in_array($cuentaoriginal,$restados))
		{
			...
			$s810="select cue_nombre from ".$_SESSION['bdd']."_sgcaf810 where cue_codigo='$cuenta'";
...
if ($accion == "CancelarPrestamo") {	//  para cancelar prestamos
	$mostrarregresar=1;
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='pres_ahorropro.php?accion=CancelaPrestamo' name='form1' id='form1' method='post' onsubmit='return valajuste(form1)'";
	$cedula = $_GET['cedula'];
	$elnumero = $_GET['nropre'];
	$micedula=substr($cedula,0,4).'.'.substr($cedula,4,3).'.'.substr($cedula,7,4);
	$deci=$_SESSION['deci'];
...
//	echo $sql_310;
	...
	$sql_tipos="select * from ".$_SESSION['bdd']."_sgcaf000 where tipo = 'cancelarprestamo' order by nombre";
...
//	echo '<td><input align="right" name="cuota" type="text" id="cuota" size="12" maxlength="12" value ="'.number_format($r_310['cuota_ucla'],2,'.','').'"></td>';
	$sql_acta="select * from ".$_SESSION['bdd']."_sgcafact where especial = 0 order by fecha desc limit 1";
	$las_actas=mysql_query($sql_acta);
	$el_acta=mysql_fetch_assoc($las_actas);
...
if ($accion == "CancelaPrestamo") {	//  para aquellos que solo aumentan el monto y varian la cuota
	...
	$r_200=mysql_fetch_assoc($a_200);
	// actualizo 
	$sql_310="update ".$_SESSION['bdd']."_sgcaf310 set stapre_sdp='C', renovado = 1, renova_por = '".$concepto."', paga_hasta = '$primerdcto' where (nropre_sdp = '$elnumero') and (cedsoc_sdp = '$micedula')";
//	echo $sql_310;
	$a_310=mysql_query($sql_310);
	...
	$sql="insert into ".$_SESSION['bdd']."_sgcabita (cuento, ip, quien) values ('$cuento', '$ip', '$usuario')";
...
if ($accion == "NetoDepositar") {	//  para modificar neto a depositar de los prestamos
	$mostrarregresar=1;
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='pres_ahorropro.php?accion=NetoDeposito' name='form1' id='form1' method='post' onsubmit='return valajuste(form1)'";
	$cedula = $_GET['cedula'];
	$elnumero = $_GET['nropre'];
	$micedula=substr($cedula,0,4).'.'.substr($cedula,4,3).'.'.substr($cedula,7,4);
	$deci=$_SESSION['deci'];
...
if ($accion == "NetoDeposito") {	//  para aquellos que solo aumentan el monto y varian la cuota
	...
	$sql_310="update ".$_SESSION['bdd']."_sgcaf310 set netcheque = '$neto' where (nropre_sdp = '$elnumero') and (cedsoc_sdp = '$micedula')";
//	echo $sql_310;
	$a_310=mysql_query($sql_310);
...
if ($accion == "Concretar") {	// hacer los asientos y actualizar el prestamo, faltarian los fiadores
	...
	$montoprestamo = $_SESSION['montoprestamo'];
	$micedula=substr($cedula,0,4).'.'.substr($cedula,4,3).'.'.substr($cedula,7,4);
	$sql_310="select * from ".$_SESSION['bdd']."_sgcaf310, ".$_SESSION['bdd']."_sgcaf360 where (cedsoc_sdp='$micedula') and (nropre_sdp='$elnumero') and (codpre_sdp = '$elprestamo') and (stapre_sdp='A' and ! renovado) and (codpre_sdp=cod_pres)";
//	echo $sql_310;
	$a_310=mysql_query($sql_310);
	$r_310=mysql_fetch_assoc($a_310);
	$hoy = date("Y-m-d");
...
//		echo "Generando encabezado contable <strong>$elasiento </strong> <br>";
		...
		$sql = "INSERT INTO ".$_SESSION['bdd']."_sgcaf830 (enc_clave, enc_fecha, enc_desco, enc_desc1, enc_debe, enc_haber, enc_item, enc_dif, enc_igual, enc_refer, enc_sw, enc_explic) VALUES ('$elasiento', '$b', '$desc','',0,0,0,0,0,0,0,'$desc')"; 
...
*/
			if (!empty($$variable)) 
			{
				...
				if ((substr($$variable,0,2)!='SC')) {
					$s310="select cuent_pres, codsoc_sdp, descr_pres, cuent_int from ".$_SESSION['bdd']."_sgcaf310, ".$_SESSION['bdd']."_sgcaf360 where (cedsoc_sdp='$micedula') and (nropre_sdp = '".$$variable."') and (stapre_sdp='A' and ! renovado) and (codpre_sdp=cod_pres)";
				...
				// saldo pendiente del prestamo
					...
					$sql_312="insert into ".$_SESSION['bdd']."_sgcaf312 (tipo, cuento, cuenta, monto, numero, cedula) VALUES ('$cargar','Canc.".$r310['descr_pres']."', '$cuenta1', $debe, '$elnumero','$micedula')";
					...
					$sql_312="insert into ".$_SESSION['bdd']."_sgcaf312 (tipo, cuento, cuenta, monto, numero, cedula) VALUES ('$cargar','Int.".$r310['descr_pres']."', '$cuenta1', $debe, '$elnumero','$micedula')";
					$resultado=mysql_query($sql_312);
				// cancelar el prestamo
					...
					$sql="update ".$_SESSION['bdd']."_sgcaf310 set renovado = 1, renova_por = '$arenovar', paga_hasta='$primerdcto' where nropre_sdp='$numero'" ;
					echo $sql.' 1 <br>';
					...
					if ($debe!=0.00) {
						$sql_312="insert into ".$_SESSION['bdd']."_sgcaf312 (tipo, cuento, cuenta, monto, numero, cedula) VALUES ('$cargar',' Pago Saldo ".$cuenta."', '$cuenta1', $debe, '$elnumero','$micedula')";
//						echo $sql_312;
						$resultado=mysql_query($sql_312);
					}
		...
		if ($r_360['pla_autor'] == 0)
			...
			$sql_deduccion="select * from ".$_SESSION['bdd']."_sgcaf311 where activar = 2";
		...
		while($r_deduccion=mysql_fetch_assoc($a_deduccion)) {
			...
			$sql_312="insert into ".$_SESSION['bdd']."_sgcaf312 (tipo, cuento, cuenta, monto, numero, cedula) VALUES ('-','".$r_deduccion['cuento']."', '$cuenta1', $monto_deduccion, '$elnumero','$micedula')";
//			echo $sql_312;
			$resultado=mysql_query($sql_312);
		}
		...
		if ($debe != 0) {
			if ($r_360['otractaab']=='')
			{
				$sql="select * from ".$_SESSION['bdd']."_sgcaf000 where tipo='CtaSocxPag'";
...
//			echo 'cuenta mostrada '.$cuenta1.'<br>';
		...
		for ($i=0;$i<$_POST['registrosf'];$i++)
		{
			...
			if ($$cedfia != '')
			{
				...
				if ($$montofia > 0) 
				{
					$sqlfia="insert into ".$_SESSION['bdd']."_sgcaf320 
...
//	echo $elnumero. ' - '.$elregistro;
	$sql_320="delete from ".$_SESSION['bdd']."_sgcaf320 where registro='$elregistro'";
...
//	echo 'guardo el fiador y vuelvo a solicitar '.$elnumero. ' - ' .$lacedula;
	$sql_200="select cod_prof from ".$_SESSION['bdd']."_sgcaf200 where ced_prof = '$cedulafiador'";
//	echo $sql_200;
	$a_200=mysql_query($sql_200);
	$r_200=mysql_fetch_assoc($a_200);
	$sql_310="select cod_prof from ".$_SESSION['bdd']."_sgcaf200, ".$_SESSION['bdd']."_sgcaf310 where (nropre_sdp='$elnumero') and (cod_prof = codsoc_sdp)";
//	echo $sql_310;
	$a_310=mysql_query($sql_310);
	$r_310=mysql_fetch_assoc($a_310);
	if (mysql_num_rows($a_200) > 0) // existe el fiador
	{
		...
		$ip = $_SERVER['HTTP_CLIENT_IP'];
		if (!$ip) {$ip = $_SERVER['REMOTE_ADDR'];}
		$sql_320="insert into ".$_SESSION['bdd']."_sgcaf320 (codsoc_fia, nropre_fia, codfia_fia, monlib_fia, tipmov_fia, monto_fia, ip) values 
...
if ($mostrarregresar==1) { // ($accion == "Buscar") or ($accion == "Ver") or ($accion="EscogePrestamo")) {
	echo '<form enctype="multipart/form-data" name="formdepie" method="post" action="pres_ahorropro.php?accion=Buscar">';
...
function solicitar_fiadores($elnumero,$lacedula)
{
	...
	echo "<form enctype='multipart/form-data' action='pres_ahorropro.php?accion=Fiadores' name='form1' id='form1' method='POST' onsubmit='return valfiadores(form1)'>";
	// &elnumero=$elnumero&cedula=$cedula&
	echo "<form action='pres_ahorropro.php?accion=Buscar' name='form1' method='post'>";
	$micedula=substr($lacedula,0,4).'.'.substr($lacedula,4,3).'.'.substr($lacedula,7,4);
	$sql_310="select * from ".$_SESSION['bdd']."_sgcaf310, ".$_SESSION['bdd']."_sgcaf360, ".$_SESSION['bdd']."_sgcaf200 where (cedsoc_sdp='$micedula') and (nropre_sdp='$elnumero') and (stapre_sdp='A' and ! renovado) and (codpre_sdp=cod_pres) and (ced_prof='$lacedula') limit 1";
...
//	$sql_320="select * from sgcaf320, sgcaf200 where (nropre_fia = '$elnumero') and (codsoc_fia='$elcodigo') and (ced_prof='$cedula')";
	$sql_320="select * from ".$_SESSION['bdd']."_sgcaf320, ".$_SESSION['bdd']."_sgcaf200 where (nropre_fia = '$elnumero') and (codfia_fia=cod_prof)";
//	echo $sql_320;
	$a_320=mysql_query($sql_320);
...
//			echo '<td class="centro azul"><input type="checkbox" id="eliminar'.$registros.'" name="eliminar'.$registros.'" value='.$r_320["registro"] .' onClick="eliminafianza()"> </td>' ;
		...
		echo "<a href='pres_ahorropro.php?accion=Eliminar&cedula=".$lacedula."&nropre=".$elnumero."&registro=".$r_320['registro']."& '  onClick='return conf_elim_fiadores()'>";
...
//	echo 'el asiento '.$asiento.'<br>';
	$sql_f820="select com_monto1, com_monto2 from ".$_SESSION['bdd']."_sgcaf820 where com_cuenta='$cuenta' ";
	if ($asiento == '')
		$sql_f820.="";
	else
...
*/			$factor = 12;
	...
	$sql_360="select * from ".$_SESSION['bdd']."_sgcafapc where '".substr($tipo,0,1)."'=serie";
	$a_360=mysql_query($sql_360);
	$r_360=mysql_fetch_assoc($a_360);
/*
	$sql_310="select * from ".$_SESSION['bdd']."_sgcaf310, ".$_SESSION['bdd']."_sgcaf360 where cedsoc_sdp='$micedula' and nropre_sdp='$nropre'";
	$a_310=mysql_query($sql_310);
	$r_310=mysql_fetch_assoc($a_310);
*/
		echo '<fieldset><legend>'.trim($tipo). ' / '.trim($r_200['ape_prof']). ', '.trim($r_200['nombr_prof']).' / ';
	echo $r_200['ced_prof'].' / '.$r_200['cod_prof'].' / '.$elnumero;
	if 	($_SESSION['numeroarenovar']) echo ' <br>(Renovacion) ';
	echo '</legend>';
	echo '<table align="center" class="basica 100 hover" width="500" border="1">';
	echo '<tr>';
    echo '<td width="100">Inicia el </td><td width="100" align="right">'.convertir_fechadmy($r_360['vdesde']).'</td>';
	echo "<input type = 'hidden' value ='".$r_200['ced_prof']."' name='cedula' id='cedula'>";
    echo '<td width="100">Finaliza el </td><td width="100" align="right">'.convertir_fechadmy($r_360['vhasta']).'</td>';
/*
    echo '<td width="150">Monto Solicitado </td><td width="100" align="right">';
	// -----------
	$s_100="select ut from ".$_SESSION['bdd']."_sgcaf100 limit 1";
	...
	if ($_SESSION['disponibilidadprestamo'] <= 0)
		if ($r_360['tope_ut'] == 0)
			...
			if ($r_360['e_items'] == 1)
			{
				$s_items="select sum(monpre_sdp-monpag_sdp) as saldo from ".$_SESSION['bdd']."_sgcaf310 where cedsoc_sdp='$micedula' and codpre_sdp='$tipo' and stapre_sdp='A' and (! renovado) group by cedsoc_sdp";
...
//	---------------
    ...
    echo '<td>Monto Prestamo </td><td  align="right">'.number_format($r_360['prestamo'],$deci,$sep_decimal,$sep_miles).'</td></tr>';
/*
	echo '<tr>';
...
//		echo "<input type = 'text' value ='".$primerdcto."' name='primerdcto' id='primerdcto'>";
		$sql_acta="select * from ".$_SESSION['bdd']."_sgcafact where especial = 1 order by fecha desc limit 3";
		$las_actas=mysql_query($sql_acta);
//		echo '111';
...
*/
	...
	echo '<select id="lascuotas" name="lascuotas" size="1" disabled>';
	for ($laposicion=$r_360['cuotas'];$laposicion >= 1;$laposicion--) {
		echo '<option value="'.$laposicion.($posicion==$r_360['cuotas']?" selected ":"").'">'.$laposicion.'  </option>'; }
		// 
	echo '</select>'; 
	echo '</td>';
    echo '<td>Cuota Original </td><td  align="right">';
	// .number_format($r_310['cuota'],$deci,$sep_decimal,$sep_miles).;
	echo '<input align="right" name="cuota" type="text" id="cuota" size="12" maxlength="12" readonly="readonly" value ="'.number_format(($r_360['prestamo']/$r_360['cuotas']),2,'.',',').'">';
/*
...
/*
	...
	echo '>';
	echo '</td></tr>';
*/	echo '<tr><td>Acta / Fecha </td><td>'.$nroacta.' del '.convertir_fechadmy($fechaacta).'</td>';
...
function mostrar_prestamo($cedula,$nropre)
{
	...
	$r_200=mysql_fetch_assoc($a_200);
	$micedula=substr($cedula,0,4).'.'.substr($cedula,4,3).'.'.substr($cedula,7,4);
	$sql_310="select * from ".$_SESSION['bdd']."_sgcaf310, ".$_SESSION['bdd']."_sgcaf360 where cedsoc_sdp='$micedula' and nropre_sdp='$nropre' and (codpre_sdp=cod_pres)";
	$a_310=mysql_query($sql_310);
	$r_310=mysql_fetch_assoc($a_310);
...
function actualizar_acta($nroacta, $monto, $primerdcto) {
	$sql="update ".$_SESSION['bdd']."_sgcafact set eje_pre=eje_pre + $monto where ((acta ='$nroacta') and (f_dcto = '$primerdcto'))";
	$resultado=mysql_query($sql);
}
