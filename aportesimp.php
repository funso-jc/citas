...
*/
$linea=0;
// encabeza_aportes($pdf,$linea);
// echo $elarchivo. ' - '.$proceso;
$linea=15;
$pdf->SetY($linea);
$pdf->SetX(0);
$concepto='A';
$pdf->SetFont('Arial','B',8);
$pdf->MultiCell(0,0,"Listado de Retención/Aportes al ".$fechaaporte,0,C,0);
$pdf->SetY($linea);
$pdf->SetFont('Arial','',8);
$pdf->SetFillColor(255,255,255);
$pdf->SetTextColor(0);
$linea+=10;
$header=array('Código','Cédula','Apellidos y Nombres','Retención','Aporte','Total');
$w=array(10,20,60,20,20,20);
...
$sql="select * from sgcanomi where archivo='$elarchivo' and proceso='$proceso' order by cedula";
...
while ($row = mysql_fetch_array($result))
{
 ...
 //imprimo nombre, apellidos y localidad
	...
	$pdf->SetY($linea);
	$pdf->SetX(30);
	$pdf->Cell($w[0],5,$row['socio'],0,0,'LRTB',0); 
	$pdf->Cell($w[1],5,$row['cedula'],0,0,'LRTB',0); 
	$cont++; 
	$acotado = trim($row["nombre"]);
    $pdf->Cell($w[2],5,$acotado,0,0,'LRTB',0);  
	$pdf->Cell($w[4],5,number_format($row[monto],2,".",","),0,0,'R',0);
	$pdf->Cell($w[5],5,number_format($row[monto2],2,".",","),0,0,'R',0);
	$pdf->Cell($w[5],5,number_format($row[monto]+$row[monto2],2,".",","),0,0,'R',0);
	$emonto+=$row[monto];
	$emonto2+=$row[monto2];
	if ($linea >= $maxlineas)
	{
		...
		$pdf->SetY($linea);
		$pdf->SetX($w[2]);
	    $pdf->Cell($w[2],5,'Van....',0,0,'R',0);  
		$pdf->Cell($w[4],5,number_format($emonto,2,".",","),0,0,'R',0);
		$pdf->Cell($w[5],5,number_format($emonto2,2,".",","),0,0,'R',0);
		$pdf->Cell($w[5],5,number_format($emonto+$emonto2,2,".",","),0,0,'R',0);
		$pdf->AddPage();
//		encabeza_aportes($pdf,$linea);
	$linea=15;
	$pdf->SetY($linea);
...
$pdf->Cell($w[2],5,'Total.... ' .$cont,0,0,'R',0);  
$pdf->Cell($w[4],5,number_format($emonto,2,".",","),0,0,'R',0);
$pdf->Cell($w[5],5,number_format($emonto2,2,".",","),0,0,'R',0);
$pdf->Cell($w[5],5,number_format($emonto+$emonto2,2,".",","),0,0,'R',0);
$pdf->Output();
